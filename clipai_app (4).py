...
Rules: never exceed {duration:.0f}s, no overlaps, each clip ~{clip_length}s."""
    ...
    # Extract the JSON array — find the first [ and last ] to handle extra text before/after
    start_idx = text.find("[")
    end_idx = text.rfind("]")
    ...
    try:
        ...
        raise Exception(f"AI returned invalid JSON: {str(e)} — raw response: {text[:300]}")
