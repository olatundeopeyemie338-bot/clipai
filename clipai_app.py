...
def generate_captions_srt(start, end, job_id, api_key):
    """Ask AI to generate fake but realistic caption timings for the clip"""
...
def cut_clip(video, start, end, output, add_captions, caption_style, do_reframe,
    ...
    if add_captions:
        try:
            ...
            api_key = os.environ.get("OPENROUTER_API_KEY","")
            srt_content = generate_captions_srt(start, end, None, api_key)
            if srt_content:
    ...
    try:
        ...
        for i, c in enumerate(clips_data[:num_clips]):
            ...
            cut_clip(video_path, start, end, os.path.join(job_dir, fn),
                     add_captions, caption_style, do_reframe, do_mirror, do_colorgrade, do_speed, do_pitch, add_music)
