...
.clip-top { padding: 14px 14px 0; }
.clip-title { font-size: 0.9rem; font-weight: 700; margin-bottom: 6px; }
...
.clip-reason { font-size: 0.78rem; color: #64748b; line-height: 1.5; margin-bottom: 12px; padding: 0 14px; }
...
.err { color: #f87171; text-align: center; margin-top: 12px; display: none; font-size: 0.9rem; padding: 12px; background: #1a0000; border-radius: 8px; }
</style>
</head>
<body>
...
document.getElementById('file').onchange=e=>{
  if(e.target.files[0]){
    document.getElementById('chosen').textContent='✅ '+e.target.files[0].name;
...
async function poll(){
  try{
    ...
    if(d.step==='done'&&d.clips&&d.clips.length>0){
      clearInterval(timer);
      showResults(d.clips);
...
function showResults(clips){
  ...
  list.innerHTML=clips.map((c,i)=>{
    const score=c.virality_score||70;
    const bc=score>=75?'high':score>=50?'mid':'low';
    const em=score>=75?'🔥':score>=50?'⚡':'📊';
    return `<div class="clip-item">
      <div class="clip-top">
        <div class="clip-title">${i+1}. ${c.title}</div>
        <div class="clip-meta">
          <span class="badge ${bc}">${em} ${score}/100 Virality</span>
          <span class="tag2">⏱ ${c.duration}s</span>
          <span class="tag2">📐 9:16</span>
        </div>
      </div>
      <div class="clip-reason">${c.reason}</div>
      <a class="dl-btn" href="/download/${jobId}/${c.filename}" download="${c.filename}">
        ⬇️ Download Clip ${i+1}
      </a>
    </div>`;
  }).join('');
...
def download_clip(job_id, filename):
    ...
    if not os.path.exists(filepath):
        return jsonify({"error": "File not found — it may have expired. Please regenerate."}), 404
...
def analyze_with_claude(duration, num_clips, clip_length, job_id):
    ...
    prompt = f"""A video is {duration:.0f} seconds long. Generate exactly {num_clips} clip suggestions, each ~{clip_length} seconds long. Space them evenly across the video.

Return ONLY a valid JSON array. No markdown, no explanation:
[
  {{"title":"Catchy clip title","start":0,"end":{clip_length},"virality_score":80,"reason":"One sentence why this is engaging"}}
]

Rules:
- Never go past {duration:.0f} seconds
- Each clip must be ~{clip_length} seconds
...
def cut_clip(video, start, end, output):
    ...
    subprocess.run(["ffmpeg","-ss",str(start),"-i",video,"-t",str(duration),
...
def process_video(job_id, video_path, job_dir, num_clips, clip_length):
    try:
        ...
        for i, c in enumerate(clips_data[:num_clips]):
            ...
            out = os.path.join(job_dir, fn)
            jobs[job_id]["step"] = "clips"
            cut_clip(video_path, start, end, out)
        ...
        jobs[job_id].update({
            "status":"error","step":"done",
            "error_step": jobs[job_id].get("step","clips"),
            "message": str(e)
        })
