...
UPLOAD_FOLDER = "/home/claude/clipper/uploads"
OUTPUT_FOLDER = "/home/claude/clipper/outputs"
...
def cut_clip_with_captions(video_path, start, end, output_path, caption_segments):
    ...
    if orig_w / orig_h > target_aspect:
        ...
        x_off = 0
        y_off = max(0, (orig_h - new_h) // 2)

    # Build caption filter if we have segments
    filter_complex = f"crop={new_w}:{new_h}:{x_off}:{y_off},scale=1080:1920"

    if caption_segments:
        # Filter to captions in this clip's time range
        relevant = [s for s in caption_segments if s["end"] > start and s["start"] < end]
        if relevant:
            drawtext_parts = []
            for seg in relevant[:30]:  # limit
                seg_start = max(0, seg["start"] - start)
                seg_end = min(duration, seg["end"] - start)
                ...
                drawtext_parts.append(
                    ...
                    f":enable='between(t,{seg_start:.2f},{seg_end:.2f})'"
                )
            if drawtext_parts:
...
def process_video(job_id, video_path, job_dir, num_clips, clip_length):
    try:
        ...
        caption_segments = transcribe_audio(audio_path)
        ...
        jobs[job_id]["step"] = "analyze"
        clips_data = analyze_with_claude(transcript_text, duration, num_clips, clip_length, job_id)
        ...
        for i, clip in enumerate(clips_data[:num_clips]):
            ...
            filename = f"clip_{i+1:02d}_score{clip['virality_score']}.mp4"
            output_path = os.path.join(job_dir, filename)

            jobs[job_id]["step"] = "clips"
            cut_clip_with_captions(video_path, start, end, output_path, caption_segments)
