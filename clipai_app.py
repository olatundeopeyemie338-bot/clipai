    ...
    try:
        ...
        t = threading.Thread(target=process_video,
                             args=(job_id, video_path, job_dir, num_clips, clip_length,
                                   topic, cp_level, add_captions, do_reframe,
...
def cut_clip(video, start, end, output, add_captions, do_reframe,
    ...
    if add_captions:
        filters.append("drawtext=text='':fontsize=1:fontcolor=white:x=0:y=0")
    ...
    if add_music:
        audio_filters.append("volume=0.85")

    if has_logo:
        # Use filter_complex to overlay logo
        logo_filter = f"[1:v]scale=160:-1,format=rgba,colorchannelmixer=aa=0.75[logo];[0:v]{vf if vf else 'null'}[base];[base][logo]overlay=W-w-20:H-h-20"
        cmd = [ffmpeg_exe, "-ss", str(start), "-i", video, "-i", logo_path, "-t", str(duration),
    ...
    try:
        ...
        for i, c in enumerate(clips_data[:num_clips]):
            ...
            cut_clip(video_path, start, end, os.path.join(job_dir, fn),
                     add_captions, do_reframe, do_mirror, do_colorgrade, do_speed, do_pitch, add_music)
