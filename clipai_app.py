    ...
    try:
        ...
        if video_url:
            ...
            for cmd in ["yt-dlp", "python3 -m yt_dlp"]:
                r = subprocess.run(["which", cmd.split()[0]], capture_output=True)
                if r.returncode == 0:
                    yt_dlp_cmd = cmd.split()[0]
                    break
            if not yt_dlp_cmd:
                yt_dlp_cmd = "yt-dlp"  # try anyway
            result = subprocess.run([
                yt_dlp_cmd, "-f", "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best",
                "--merge-output-format", "mp4",
                "-o", video_path, video_url,
                "--no-playlist", "--max-filesize", "500m",
                "--ffmpeg-location", os.path.dirname(get_ffmpeg_exe()),
            ...
            if result.returncode != 0 or not os.path.exists(video_path):
                return jsonify({"error": f"Could not download video: {result.stderr[-300:] if result.stderr else 'Unknown error'}"}), 400
        else:
