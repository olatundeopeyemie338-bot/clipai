...
.meta-box { margin: 0 12px 10px; background: #13132a; border: 1px solid #2d2d4e; border-radius: 10px; padding: 10px 12px; }
.meta-label { font-size: 0.68rem; color: #64748b; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 4px; display: flex; justify-content: space-between; align-items: center; }
.meta-text { font-size: 0.8rem; color: #e2e8f0; line-height: 1.5; word-break: break-word; }
.copy-btn { background: #2d2d4e; border: none; color: #a78bfa; font-size: 0.7rem; font-weight: 700; padding: 3px 8px; border-radius: 6px; cursor: pointer; flex-shrink: 0; }
.copy-btn:active { background: #3b1d8a; }
...
<div class="box">
  ...
  <div style="background:#1a1a2e;border:1px solid #2d2d4e;border-radius:10px;padding:12px 14px;margin-bottom:4px;display:flex;align-items:flex-start;gap:10px">
    <span style="font-size:1.2rem">📱</span>
    <div>
      <div style="font-size:0.82rem;font-weight:700;color:#e2e8f0;margin-bottom:3px">Upload your video file</div>
      <div style="font-size:0.75rem;color:#64748b;line-height:1.5">Download your video from YouTube, TikTok, Instagram or any platform first, then upload it above. URL links are not supported.</div>
    </div>
  </div>
...
let useUrlMode = false; // always false - upload only
...
def upload():
    try:
        ...
        if video_url:
            return jsonify({"error": "URL downloading is not supported. Please download your video from YouTube, TikTok or Instagram first, then upload the file directly."}), 400
        else:
            ...
            if not os.path.exists(video_path) or os.path.getsize(video_path) < 1000:
                return jsonify({"error": "File upload failed or file is too small. Please try again."}), 400
...
def get_duration(path):
    ...
    try:
        fsize = os.path.getsize(path) if os.path.exists(path) else -1
        r2 = subprocess.run([ffmpeg_exe,"-i",path], capture_output=True, text=True, timeout=10)
        debug = (r2.stdout + r2.stderr)[:300]
    except Exception as de:
        fsize = -1; debug = str(de)
    raise Exception(f"Could not get video duration — file size: {fsize} bytes, ffmpeg output: {debug}")
...
Rules: never exceed {duration:.0f}s, no overlaps, each clip ~{clip_length}s."""
    ...
    data = json.dumps({"model":"meta-llama/llama-3.1-8b-instruct","max_tokens":1500,
                       "messages":[{"role":"user","content":prompt}]}).encode()
    req = ur.Request("https://openrouter.ai/api/v1/chat/completions", data=data,
                     headers={"Content-Type":"application/json",
                               "Authorization":f"Bearer {api_key}",
    ...
    # Robustly extract JSON array using first [ and last ]
    start_idx = text.find("[")
    end_idx = text.rfind("]")
    ...
    try:
        ...
        raise Exception(f"AI returned invalid JSON: {str(e)} — raw: {text[:300]}")
...
def cut_clip(video, start, end, output, add_captions, caption_style, do_reframe,
    ...
    if do_reframe:
        if w > h:
            # Landscape — crop sides to get 9:16 portrait
            nw = int(h * 9 / 16)
            x = (w - nw) // 2
            filters.append(f"crop={nw}:{h}:{x}:0")
        else:
            # Already portrait — crop top/bottom if needed
            nh = int(w * 16 / 9)
            y = max(0, (h - nh) // 2)
