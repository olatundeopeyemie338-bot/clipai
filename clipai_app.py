...
.section-title { font-size: 0.8rem; color: #94a3b8; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; margin: 20px 0 10px; }
.row { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
.full { grid-column: 1 / -1; }
label { display: block; font-size: 0.8rem; color: #94a3b8; margin-bottom: 6px; font-weight: 600; }
select, input[type=text] { width: 100%; background: #0f0f1a; border: 1px solid #2d2d4e; border-radius: 8px; color: #e2e8f0; padding: 10px; font-size: 0.9rem; }
.topics { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 4px; }
.topic { background: #1e1e2e; border: 1px solid #2d2d4e; color: #94a3b8; padding: 6px 14px; border-radius: 20px; cursor: pointer; font-size: 0.8rem; transition: all 0.2s; }
.topic.active { background: #3b1d8a; border-color: #a78bfa; color: white; }
.toggle-row { display: flex; align-items: center; justify-content: space-between; background: #0f0f1a; border: 1px solid #1e1e2e; border-radius: 10px; padding: 12px 14px; margin-top: 10px; }
.toggle-label { font-size: 0.85rem; font-weight: 600; }
.toggle-sub { font-size: 0.75rem; color: #64748b; }
.toggle { position: relative; width: 44px; height: 24px; flex-shrink: 0; }
.toggle input { opacity: 0; width: 0; height: 0; }
.slider { position: absolute; inset: 0; background: #2d2d4e; border-radius: 24px; cursor: pointer; transition: 0.3s; }
...
.btn { width: 100%; margin-top: 18px; padding: 14px; background: linear-gradient(135deg, #7c3aed, #db2777); border: none; border-radius: 12px; color: white; font-size: 1rem; font-weight: 700; cursor: pointer; }
...
.err { color: #f87171; text-align: center; margin-top: 12px; display: none; font-size: 0.82rem; padding: 12px; background: #1a0000; border-radius: 8px; border: 1px solid #7f1d1d; word-break: break-word; }
</style>
</head>
<body>
...
<div class="hero">
  ...
  <p>AI finds best moments · Captions · 9:16 · Virality score</p>
</div>
<div class="box">
  ...
  <div class="section-title">Clip Settings</div>
  <div class="row">
  ...
  <div class="toggle-row">
    <div><div class="toggle-label">💬 Auto Captions</div><div class="toggle-sub">Burns subtitles into video</div></div>
    <label class="toggle"><input type="checkbox" id="captions" checked/><span class="slider"></span></label>
  </div>
  <div class="toggle-row" style="margin-top:8px">
    <div><div class="toggle-label">🎵 Background Music</div><div class="toggle-sub">Adds soft background track</div></div>
    <label class="toggle"><input type="checkbox" id="music"/><span class="slider"></span></label>
  </div>
  <div class="toggle-row" style="margin-top:8px">
    <div><div class="toggle-label">📐 9:16 Reframe</div><div class="toggle-sub">Optimized for TikTok/Reels</div></div>
...
const STEPS=[
  ...
  {key:'clips',label:'Cutting & processing clips',icon:'✂️'},
  {key:'captions',label:'Adding captions & music',icon:'💬'},
...
function showResults(clips){
  ...
  document.getElementById('cliplist').innerHTML=clips.map((c,i)=>{
    ...
    const em=s>=75?'🔥':s>=50?'⚡':'📊';
    const tags = ['📐 9:16', c.has_captions?'💬 Captions':'', c.has_music?'🎵 Music':''].filter(Boolean).join('</span><span class="tag2">');
    return `<div class="clip-item"><div class="clip-top">
      <div class="clip-title">${i+1}. ${c.title}</div>
      <div class="clip-meta"><span class="badge ${bc}">${em} ${s}/100</span><span class="tag2">⏱ ${c.duration}s</span><span class="tag2">${tags}</span></div>
...
def upload():
    try:
        ...
        t = threading.Thread(target=process_video,
                             args=(job_id, video_path, job_dir, num_clips, clip_length, topic, add_captions, add_music, do_reframe))
        t.daemon = True
        t.start()
...
def generate_silence(output, duration):
    ffmpeg_exe = get_ffmpeg_exe()
    subprocess.run([ffmpeg_exe,"-f","lavfi","-i",f"anullsrc=r=44100:cl=stereo",
                    "-t",str(duration),"-q:a","9","-acodec","libmp3lame",
                    output,"-y","-loglevel","error"], check=True)

def analyze_with_ai(duration, num_clips, clip_length, topic, job_id):
...
def cut_clip(video, start, end, output, add_captions=False, add_music=False, do_reframe=True):
    ffmpeg_exe = get_ffmpeg_exe()
    duration = end - start
    ...
    if do_reframe:
        if w/h > 9/16:
            ...
            crop = f"crop={nw}:{h}:{x}:0,scale=720:1280"
        else:
            nh = int(w*16/9); y = max(0,(h-nh)//2)
            crop = f"crop={w}:{nh}:0:{y},scale=720:1280"
    else:
        crop = "scale=720:1280"

    # Add captions as simple text overlay if requested
    if add_captions:
        crop += ",drawtext=text='ClipAI':fontsize=28:fontcolor=white:borderw=2:bordercolor=black:x=(w-text_w)/2:y=h-80:enable='between(t,0,3)'"

    cmd = [ffmpeg_exe,
           "-ss", str(start), "-i", video,
           "-t", str(duration),
           "-vf", crop,
...
def process_video(job_id, video_path, job_dir, num_clips, clip_length, topic, add_captions, add_music, do_reframe):
    try:
        ...
        clips_data = analyze_with_ai(duration, num_clips, clip_length, topic, job_id)
        ...
        for i, c in enumerate(clips_data[:num_clips]):
            ...
            cut_clip(video_path, start, end, os.path.join(job_dir, fn),
                     add_captions=add_captions, add_music=add_music, do_reframe=do_reframe)
